...
- [ADR-0037: Agent-loop preflights headless edit permission before the first paid iteration](adr-0037-agent-loop-permission-preflight.md) — `/servo:agent-loop` (`loop.py`) subprocesses `claude -p --agent runner` with **no** `--dangerously-skip-permissions` and no `acceptEdits` / allow-list. (2026-08-27, Proposed)
