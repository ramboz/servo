...
**Run-id format.** `run-id` is a second-precision timestamp prefix + short random suffix, e.g. `20260519T143205-a3f1`. The prefix uses `%Y%m%dT%H%M%S` (15 chars; no millisecond field). Human-readable, sortable across runs without parsing, collision-resistant under spec 005's parallel-variant scenario.

**Run-id collision policy.** When `loop.py` attempts `mkdir(<target>/.servo/runs/<run-id>/, exist_ok=False)` and fails because the directory already exists, the loop driver regenerates the random suffix (timestamp prefix stays the same to preserve sortability within the second) and retries — up to **3 attempts total** (initial + 2 retries). After 3 failed attempts, the run refuses with rc=2, `reason=run_id_collision`, and a stderr breadcrumb naming the colliding run-id(s). 3 is the cap rather than open-ended retry because three collisions on the same second are pathological for a 16-bit suffix space (~65k entropy) — much more likely a stuck clock, a hostile filesystem, or a logic bug in the caller than legitimate concurrency. A bounded retry surfaces the underlying problem instead of masking it. If spec 005's parallel-variant fan-out ever surfaces a real collision rate that the seconds window can't absorb, bumping to millisecond precision (`%Y%m%dT%H%M%S%f` truncated, or `time.time_ns()`) is the documented next step — it widens the entropy window without changing the suffix shape.
...
- **`run-id` is not globally unique by construction.** The second + 4-hex-char-suffix shape gives ~65,000 distinct IDs per second, which is enough for any plausible parallel race but not a UUID. Directory-creation collisions trigger the bounded retry described in the Decision section (3 attempts then refuse); the same second getting hit three times is treated as pathological rather than something to grind through silently.
