...
The `capture_lib.mjs` extraction is disclosed as behaviour-preserving and confirmed not part of the freeze hash; the two deferred robustness items (silently-optional `setup`, the 1600x1600 constant) are disclosed with resolution triggers and were explicitly accepted as non-blocking by the compliance reviewer.
