...
| [008-eval-authoring](008-eval-authoring/spec.md) | **DRAFT scope-capture, parked; slices 008-01..04 fleshed with resolution triggers 2026-07-01.** Human-in-the-loop front-end that turns an eval-able `residual_judgment` AC into an [ADR-0005](../decisions/adr-0005-eval-oracle-component.md) frozen eval component: triage, rubric shaping, statistical reference-set collection, and frozen `n`/`δ`/threshold/judge-model — then hands off to `/servo:spec-oracle`. Entirely servo-owned per [ADR-0019](../decisions/adr-0019-eval-authoring-servo-owned.md) — no jig-side authoring step, same shape as `/servo:design-eval`. Activates on the first real EDD spec (same trigger as ADR-0005). |
...
| [020-content-fidelity-eval](020-content-fidelity-eval/spec.md) | **DRAFT.** The second eval kind ADR-0009 anticipated: extracts design-eval's already-modality-agnostic freeze/hash/aggregate/ledger/install harness into a shared `skills/_common/fidelity_eval.py` ([ADR-0024](../decisions/adr-0024-extract-frozen-eval-harness.md)), then ships `/servo:content-fidelity` — a sibling skill compiling "does this text match the rubric?" into a frozen `score_content_fidelity` component judged by a pinned text model. |
