...
| [026-design-eval-browser-acquisition](026-design-eval-browser-acquisition/spec.md) | **DONE (2026-08-19) — but read the spec's status banner.** Implements [ADR-0031](../decisions/adr-0031-design-eval-browser-acquisition.md). **Shipped:** 026-01, the non-interactive runtime preflight (the ADR's *primary* mechanism — it runs on the machine that actually hits the wall, CI/Routines, where no human is present to ask), plus salience-ranked stderr surfacing; and 026-03, per-screen browser-identity attestation in the ledger (observability, not a gate). **Not shipped:** 026-02 `DEFERRED` — the footprint concern that motivated the ADR turns on a `playwright` vs `playwright-core` question a repo that ships no browser cannot answer (ADR-0031 kill criterion 1); and 026-04 `ABANDONED` after two re-scopes, both net-negative. The DONE rollup therefore overstates coverage: the adopter-facing footprint concern remains open. |
| [012-design-eval](012-design-eval/spec.md) | **DONE (012-01..04; reviewed + reconciled 2026-08-18); 012-05 `DEFERRED`.** Compile UI-vs-mockup intent into a frozen `score_design_fidelity` oracle component (pinned vision model, n-sampled, confidence lower bound), riding ADR-0005's frozen-eval contract ([ADR-0009](../decisions/adr-0009-design-fidelity-eval-recipe.md)). The non-deterministic sibling of the deterministic component templates. The skill has shipped in every release through 0.8.0 and is the code 020-01 extracted the shared harness from — but it never went through the review-evidence ceremony, and until 2026-08-18 it had no slice files, so the board indexed a shipped skill as an unstarted draft. Named gaps: `capture.mjs` uncovered, no SKILL.md surface tests, 012-02 thinly asserted. |
...
| [030-suitability-eval-signal](030-suitability-eval-signal/spec.md) | **DRAFT (2026-08-30); DoR gates on [ADR-0036](../decisions/adr-0036-frozen-evals-satisfy-suitability-signal.md) (Proposed).** Widens the suitability gate's `has_signal` to `tests OR ci OR frozen_eval`: an **approved, frozen** eval component **installed** in the oracle (registered in `install.json`'s `components`; `approval_status: "approved"` + freeze hashes in its own config) counts as the compilable signal, so a fully instrumented design-led target — the class vellum's build-to-redline consumers multiply as Figma lands as a second design source (vellum filings, 2026-08-30) — is no longer refused the unattended path for lacking tests it does not need. Unfrozen/unapproved evals never count; a judged-only signal stays `suitable` but carries a standing non-blocking deterministic-counterweight advisory (the ADR-0033 lesson). One lean slice on `suitability.py`. |
...
| [022-dual-host-release-parity](022-dual-host-release-parity/spec.md) | **DONE (022-01 landed; board-confirmed 2026-08-18).** Committed generated Claude/Codex packages, one CI-equivalent local gate, synchronized manifests, host-explicit release assets, and a marketplace-first public install path ([ADR-0028](../decisions/adr-0028-committed-dual-host-plugin-packages.md)). |
