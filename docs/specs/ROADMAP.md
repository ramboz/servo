...
| [026-design-eval-browser-acquisition](026-design-eval-browser-acquisition/spec.md) | **DRAFT (2026-08-19).** Implements [ADR-0031](../decisions/adr-0031-design-eval-browser-acquisition.md): design-eval needs a headless browser it does not ship, and had no acquisition mechanism at all. Ships a non-interactive **runtime preflight** in `score.py` (the primary fix — it runs on the machine that hits the wall), an **unfrozen** `capture.transport` + env override so adopters can reuse an installed Chrome instead of a ~150–300 MB download, browser identity in the ledger for investigability, and an opt-in authoring assist. Explicitly introduces **no new fail-closed mode**: nothing added is hashed, so an environment move can never raise `StaleError`. ADR accepted after a 4-round frame-critique that killed two earlier framings. |
