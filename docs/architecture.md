...
- `<spec_dir>/oracle/<spec-id>/` — spec-specific evidence overlay from `/servo:spec-oracle` (specs 006-01..03, co-located with the spec by [ADR-0023](decisions/adr-0023-colocate-durable-spec-oracle-artifacts.md), slice 019-02). **Resolved relative to the spec's own directory** (`docs/specs/<spec>/oracle/<slice-id>/`), not the target's `.servo/` — a spec and its oracle travel together in git/review. Contents: `plan.md` (human-reviewable AC→check map) and `checks.json` (machine plan) from the planner (006-01); `oracle.sh.fragment` (the generated `# SEED:start spec_oracle_<id>` / `# SEED:end spec_oracle_<id>` block) from the overlay compiler (006-03); and append-only `ledger.jsonl` (one JSONL evidence record per AC per run, stamped with a run `ts`). The check engine (`checks.py`) is **referenced**, not copied — the fragment invokes the shared, plugin-sibling `checks.py` at its resolved install path — unless `--vendor-engine` is passed to `generate`/`install`, which still copies a private `checks.py` alongside the plan for the documented clone-portability case (a Routine/CI that clones the repo without the servo plugin installed). Install splices the fragment into `<target>/oracle.sh` as an ordinary component — a `score_spec_oracle_<id>` function plus a `COMPONENTS` entry — so `gate.py` / `loop.py` score it with **no special-casing**: the component returns the composite check score, or rc=2 if any check env-errors. The overlay is project-owned and reviewable before `/servo:agent-loop` consumes it; uninstall removes the component but keeps the artifacts and never touches `.servo/`. Before the loop may score it the overlay must be **approved**, and the installed component runs the engine with `--enforce-freeze` — see *Spec-oracle freeze & approval* below. **Migration:** a target with a pre-019-02 `<target>/.servo/spec-oracles/<spec-id>/` install keeps working via a soft read-fallback (`oracle_overlay.py::oracle_dir_for_spec`) — no explicit migrate step is required; the new spec-relative location takes precedence once it has its own `checks.json` (e.g. after a re-plan).

- `<spec_dir>/eval/<spec-id>/` — spec-specific **eval-authoring** artifacts from `/servo:eval-authoring` (spec 008, co-located with the spec by [ADR-0023](decisions/adr-0023-colocate-durable-spec-oracle-artifacts.md), the sibling of `oracle/<spec-id>/`). **Resolved relative to the spec's own directory**, never CWD or the target's `.servo/`. Contents: `triage.json`/`triage.md` (008-01, the eval-able vs human-residual classification of the plan's `residual_judgment` bucket); then, per authored eval, an `<eval-name>/` subdir holding `config.json` (the `fidelity_eval.py`-shaped frozen definition — judge/samples/threshold/rubric/cases + post-`emit` `hashes`/`approved_content_hash`/`approval_status`), `rubric.md` + `dataset.md` (human-reviewable renderings, 008-02/03), and append-only `ledger.jsonl` (scoring + `kind:judge_audit` records, 008-06). The goal→criteria front-end (008-05) writes a spec-shaped `criteria.md`/`criteria.json` under `<spec_dir>/<goal-slug>/` that `/servo:spec-oracle`'s plan step consumes unchanged. `emit --target <dir>` freezes then **self-installs** the component (`score.py` + `fidelity_eval.py` + `config.json` copied into `<dir>/.servo/<component>/`, a `score_<component>` function spliced into `<dir>/oracle.sh`) via the shared ADR-0024 harness — exactly as the `design-eval`/`content-fidelity` presets do; there is **no** spec-006 "eval-family compile step" (see [spec 008](specs/008-eval-authoring/spec.md) Goal 5). `gate.py` scores the installed component with no special-casing.
...
The installed component runs `checks.py --enforce-freeze`, which refuses with **rc=2** (a distinct `reason` each) on: not-approved (`spec_oracle_unapproved`), explicitly marked stale (`spec_oracle_stale` — an `approval_status: "stale"` marker, hand-set or by a future re-plan step; not a live source-spec hash compare, per [ADR-0022](decisions/adr-0022-freeze-against-parsed-acs.md) — the freeze is against the *parsed AC set* only, so a living-document consumer's status/prose edits never trip it), modified generated artifact (`spec_oracle_artifact_modified`), or relaxed checks (`spec_oracle_plan_modified`, computed over the canonicalized — order-independent, whitespace-normalized — AC set). Bare `checks.py` (planning / inspection) does **not** enforce — only the installed overlay does, so authoring stays frictionless.

**Threat model.** The hashes are *tripwires for honest drift*: an iteration that incidentally edits the engine or relaxes/removes a check is caught and the oracle refuses rather than passing silently. They are not a sandbox — a runner that writes arbitrary files could rewrite both an artifact and its recorded hash. The defense against a *deliberately* self-rewriting runner is the `runner.md` constraint (it is told not to edit approved spec-oracle artifacts, and that doing so is self-grading) plus the human approval flow, consistent with [ADR-0001](decisions/adr-0001-reuse-jig-test-detector.md)'s filesystem-only trust model. v1 leniency: a check with no negative control is approved without a falsifiability proof — the planner does not yet auto-generate controls (see [refinement-todo](refinement-todo.md)).
...
The **edit-permission-wall diagnosis** (spec 031 / [ADR-0037](decisions/adr-0037-agent-loop-permission-preflight.md)) is a **post-hoc terminal-reason relabel**, not a new brake. A headless `claude -p --agent runner` in a default-permission context has its `Edit`/`Write` silently denied — it runs, costs money, and lands zero edits — yet the loop halts with a *misleading* `oracle_plateau` / `max_iterations_reached` and no fix hint. servo now tracks, per **runner** iteration only (judges are read-only, so they never count), whether the runner landed anything on disk — an **untracked-inclusive** per-invoke git delta (`_tree_snapshot`, snapshotted immediately before/after the runner invoke, before the gate call and `.servo/` write, with servo/tooling bookkeeping filtered) — into a persisted `runner_ever_edited` flag (any edit *proves* permission and disarms the diagnosis for the run) and a persisted `edit_signal_computed` flag (was the delta ever computable). At the halt the existing brakes already produce, if the runner never edited, the signal was computable, and the oracle is below threshold, the terminal reason is relabeled to `edit_permission_unavailable` (rc=2, distinct from a clean rc=0 halt) with a `terminal_breadcrumb` naming the fix (a worktree `.claude/settings.local.json` grant, or a Routine with Bash + Edit/Write granted). Because it only relabels a halt that already happened, it can never fire earlier than today and so can never lose a capable run; the oracle-below-threshold conjunct confines every signal blind-spot to already-failing runs. **Both drivers** are covered: the loop driver relabels at its `oracle_plateau`/`max_iterations_reached` halt (031-01); the goal driver computes one whole-run delta around its single `_invoke_claude_goal` and relabels at its `iteration_cap_reached`/`oracle_below_threshold` terminal (031-02) — both through one shared `_relabel_terminal_reason` helper. Cost-ceiling halts are deliberately never relabeled (running out of budget is not a permission wall). Heartbeat dispatch is unaffected: it classifies a candidate's outcome by the loop's `final_oracle_status` (still `below_threshold` for a walled run) and its summary-parseability, neither of which the relabel changes; a walled run's `edit_permission_unavailable` (rather than `oracle_plateau`) simply no longer trips the plateau→quarantine string-match (ADR-0030), which is correct — a permission wall is not a genuine plateau.
...
Each iteration emits one JSON line to stdout (carrying `iteration`, `session_id`, `cost_usd`, `cumulative_cost_usd`, `context_fill_ratio`, `oracle_exit_code`, `oracle_status`, `oracle_composite`); a final summary line carries `terminal_reason`, `iterations_completed`, `cumulative_cost_usd`, `cost_ceiling_usd`, `context_fill_threshold`, `context_fill_ratio`, `final_oracle_status`, `run_id` (and `terminal_breadcrumb` when the terminal reason is `edit_permission_unavailable` — added conditionally so other halts keep byte-identical shape, spec 031). The `schema_version: 1` first key on every line mirrors gate.py's ADR-0002 contract.
