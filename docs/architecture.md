...
| [ADR-0010](decisions/adr-0010-triage-inbox-schema.md) | Proposed | Triage-inbox state-file schema & dedupe identity (spec 011): `schema_version: 2`; ratified `finding_id`; sticky `status` lifecycle separate from a recomputed `actionable` verdict + immutable `provenance` marker (Guardrail #4); one uniform merge + retention rule; `flock` double-fire safety; reserves `outcome.cost_usd` for 011-04. Reciprocal to ADR-0004. |

### Pending (ADR candidates)

Numbers below are *hints* of the next likely allocation order, not reservations — the next accepted ADR claims the next free number (now `0011`) regardless of which candidate fires first. (ADR-0010 is now listed Proposed in the Decisions table above; its inbox-schema "Runtime artifacts" prose is finalized when it flips to Accepted at slice 011-02 close-out.)
