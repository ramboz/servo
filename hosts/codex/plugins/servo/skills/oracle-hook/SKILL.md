...
The helper is at `${PLUGIN_ROOT}/skills/oracle-hook/hook.py`. It manages two artifacts in the target:
...
```bash
python3 "${PLUGIN_ROOT}/skills/oracle-hook/hook.py" install <target>

# longer hook timeout (seconds; default 60)
python3 "${PLUGIN_ROOT}/skills/oracle-hook/hook.py" install <target> --timeout 90
```
...
```bash
python3 "${PLUGIN_ROOT}/skills/oracle-hook/hook.py" uninstall <target>
```
...
```bash
# human-readable
python3 "${PLUGIN_ROOT}/skills/oracle-hook/hook.py" status <target>

# machine-readable (schema_version, state, entry_present, script_present)
python3 "${PLUGIN_ROOT}/skills/oracle-hook/hook.py" status <target> --json
```
