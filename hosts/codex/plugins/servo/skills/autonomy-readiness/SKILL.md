...
description: >-
  Decide whether a goal's SCOPE and INITIAL PROMPT are precise and bounded enough to hand to an unattended, long-horizon agent loop — *before* any budget is burned. Emits the ADR-0029 **readiness verdict**: a closed three-state gate (`ready` / `needs_tightening` / `unsafe_for_autonomy`), fail-closed, written to `<target>/.servo/readiness/<goal-id>.json`. Human-owned — the artifact starts `approval_status: proposed` and is never auto-approved; a human reviews the scorecard and flips it to `approved`. Readiness sits **upstream of** `/servo:edd-suitability`: suitability asks "can this spec be evaluated?", this asks the prior question "is the premise safe to run unattended at all?".
---
...
The helper lives at `${PLUGIN_ROOT}/skills/autonomy-readiness/`:
...
python3 "${PLUGIN_ROOT}/skills/autonomy-readiness/readiness.py" \
