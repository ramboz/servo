...
description: |
  ...
  Fire this skill when the user wants to:
    ...
    - "set up servo" or "set up servo on this project"
    - "scaffold oracle for this project" or "scaffold an oracle"
    - "install agent loop infrastructure"
    - "install servo here"
    - "bring this repo under servo's closed-loop control"

  Do NOT fire on:

    - bare "oracle" or "score this code" — those refer to *running* an
      already-scaffolded oracle.sh, which is a future runtime skill
      (`/servo:quality-gate`), not this scaffolder.
    - "review my code" / "run my tests" / "lint this" — out of scope.
    - "set up jig" / "scaffold jig" — that's jig's `/jig:scaffold-init`,
      a sibling plugin, not this one.

  When in doubt, ask the user which servo skill they mean rather than
  invent a trigger match.
---
...
Use when the user is asking to **install servo on a project for the first time** (or re-install with `--force`). The helper is at `${CLAUDE_PLUGIN_ROOT}/skills/scaffold-init/scaffold.py`. Tier 1 (agent-loop driver) and Tier 2 (hooks + worktree race) are forward-looking — only Tier 0 is shipped in spec 001.
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/scaffold-init/scaffold.py" <target>
```
...
python3 "${CLAUDE_PLUGIN_ROOT}/skills/scaffold-init/scaffold.py" <target> --force

# Audit only (no scaffolding) — useful for previewing detection results:
python3 "${CLAUDE_PLUGIN_ROOT}/skills/scaffold-init/scaffold.py" detect <target>
```
